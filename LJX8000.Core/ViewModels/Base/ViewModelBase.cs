...
namespace LJX8000.Core.ViewModels.Base
{
    public class ViewModelBase : INotifyPropertyChanged
    {
        ...
        public bool AutoResetFlag
        {
            ...
            set
            {
                _autoResetFlag = value;
                if (value)
                {
                    _timer = new Timer(AutoResetInterval){Enabled = true, AutoReset = true};
                    _timer.Elapsed += ResetFlag;
                    _timer.Start();
                }
            }
        }
        ...
        private void ResetFlag(object sender, ElapsedEventArgs e)
        {
            AutoResetFlag = false;
            _timer.Elapsed -= ResetFlag;
        }
